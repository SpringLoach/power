...
情景：改变单选框的选项时，某个输入框变为必填/非必填。

```react
onChangeType({ target }) {
  if (target.value === 'Money') {
    this.form.couponDiscount = undefined;
    this.rules.couponDiscount = [];
  } else if (target.value === 'Discount') {
    this.rules.couponDiscount = this.rules.couponDiscountSpare;
  }
},
```
